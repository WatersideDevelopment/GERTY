...
GertyBot.prototype._loadPlugins = function () {
    if(this.settings.cellDir) {
        var cells = fs.readdirSync(this.settings.cellDir);
        for(var cell in cells) {
            this.log.debug('Checking for cell in '+this.settings.cellDir+'/'+cells[cell]);
            var cellEntry = require(this.settings.cellDir+'/'+cells[cell]+'/package.json');
            console.log(cellEntry);
            // this needs resolving down to a proper path level, and then inc'ed.
        };
    }
    this.register(new CoreCell(this));
};
