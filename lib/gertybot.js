...
/**
 * Helper message that tries to work out the best medium to answer via...
 *
 * @param {object} message
 * @param {object} reply
 */
GertyBot.prototype.reply = function (message, reply) {
    ...
    if(channel) {
        console.log(channel);
        this.postMessageToChannel(channel.name, reply.text, reply.params, function (data) {
            if(!data.okay) console.log(data);
        });
    } else {
        this.postMessageToUser(user.name, reply.text, reply.params, function (data) {
            if(!data.okay) console.log(data);
        });
    }
};
